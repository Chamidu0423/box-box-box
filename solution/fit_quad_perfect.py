...
def extract_features(strategy, laps_total, track_temp):
    ...
    for lap in range(1, laps_total + 1):
        ...
        if current_tire == 'SOFT':
            laps_S += 1
            sum_S += current_run
            sq_S += current_run * current_run
        elif current_tire == 'MEDIUM':
            laps_M += 1
            sum_M += current_run
            sq_M += current_run * current_run
        else: # HARD
            laps_H += 1
            sum_H += current_run
            sq_H += current_run * current_run
...
def solve_regression():
    ...
    def objective(params):
        ...
        # Hinge loss basically: penalize if diffs < 0.1
        penalties = 0.1 - diffs
    ...
    # 14 params:
    # 0, 1: M_base, H_base
    # 2, 3, 4: S_deg1, M_deg1, H_deg1
    # 5, 6, 7: S_deg2, M_deg2, H_deg2 (Quadratic Deg)
    # 8, 9, 10: S_td, M_td, H_td
    # 11, 12, 13: S_tb, M_tb, H_tb
    
    initial_guess = np.zeros(14)
    # Give small positive initial for deg1 and deg2
    initial_guess[2:8] = 0.01
    
    bounds = [
        (None, None), (None, None), 
        (0.0000, None), (0.0000, None), (0.0000, None), 
        (0.0000, None), (0.0000, None), (0.0000, None), 
    ...
    p = res.x
    names = ["M_base", "H_base", "S_deg1", "M_deg1", "H_deg1", "S_deg2", "M_deg2", "H_deg2", "S_td", "M_td", "H_td", "S_tb", "M_tb", "H_tb"]
