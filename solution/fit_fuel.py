...
def lap_time_sum(strategy, laps_total, track_temp, params):
    base_s, base_m, base_h, deg_s, deg_m, deg_h, temp_mult, fuel_weight = params
    ...
    for lap in range(1, laps_total + 1):
        ...
        # Fuel weight logic: Car gets lighter, so deg is lower
        # or maybe the car gets lighter, so base lap time is fundamentally faster
        fuel_mod = (laps_total - lap) * fuel_weight
        
        # Test 1: Fuel makes the whole lap slower (worse early in the race)
        if current_tire == 'SOFT':
            tire_eff = base_s + deg_s * laps_on_tire
        elif current_tire == 'MEDIUM':
            tire_eff = base_m + deg_m * laps_on_tire
        else: # HARD
            tire_eff = base_h + deg_h * laps_on_tire
            
        temp_eff = track_temp * temp_mult
        
        # NOTE: if fuel_mod just adds to lap time independent of tire, order STILL doesn't matter!
        # It HAS to interact with the compound or degradation.
        # Let's interact it with degradation:
        tire_eff = tire_eff + (tire_eff * fuel_mod)
...
def find_nonlinear_formula():
    ...
    # params: base_s, base_m, base_h, deg_s, deg_m, deg_h, temp_mult, fuel_weight
